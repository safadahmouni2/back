...
public interface SprintRepository extends JpaRepository<Sprint, Long>, JpaSpecificationExecutor<Sprint> {
	...
	@Query(value = "SELECT COALESCE(round(T.spbc / round(IF(T.sc = 0, 1, T.sc) * 100,2), 2) ,0) FROM (SELECT SUM(X.complexity * X.progress) AS spbc, SUM(X.complexity) AS sc FROM (SELECT COALESCE(US.story_points, 100) AS complexity, COALESCE(US.progress, 0) AS progress FROM pts_agile.user_story AS US WHERE US.sprint_id = ?1) AS X) AS T;", nativeQuery = true)
