...
DECLARE t_ticket_id,
		...
		t_test_step_id,
		t_parent_ticket_id,
		t_parent_us_ticket_id  BIGINT(20);

DECLARE t_order_by_ds,
		t_order_by_topic DECIMAL(31,15);
		
DECLARE t_project,
	    t_role  VARCHAR(100) CHARACTER SET UTF8;
		
DECLARE t_goal, 
		t_benefit,
		t_short_description varchar(250) CHARACTER SET UTF8;
...
DECLARE t_creator VARCHAR(250) CHARACTER SET UTF8;

DECLARE t_created TIMESTAMP ;

DECLARE t_modifier VARCHAR(250) CHARACTER SET UTF8;

DECLARE t_modified TIMESTAMP ;

DECLARE ticket_exists INTEGER ;

DECLARE t_user_code VARCHAR(10) CHARACTER SET UTF8;
...
LEFT JOIN pts.core_user AS CU ON CU.user_id = PT.responsible_id 
LEFT JOIN pts_agile.sprint AS S ON S.ticket_id = PT.matrix_id
...
INSERT INTO user_story (ticket_id,  state_id, responsible_id, product_id,  sprint_id , urgency_id , story_points, progress, feature_id, test_id, test_run_id,
 test_step_id, order_by_topic, order_by_ds, project, role, goal, benefit, short_description, long_description, acceptance_criteria,  creator, created, modifier, modified,topic_id ,parent_ticket_id,user_code,exported_at, client_ref, parent_us_ticket_id )
VALUES(
			t_ticket_id,
			t_state_id,
			t_responsible_id,
