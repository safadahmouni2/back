	...
	BEGIN
        ...
        DECLARE  cursor_data CURSOR FOR 
		    SELECT 
				...
				topic.modifier,
				topic.modified,
				COALESCE(topic.client_ref, CONCAT(KUNDE_REF_PREFIX, topic.id)) AS client_ref
			FROM topic
        ...
        read_loop: LOOP
			...
			UPDATE topic SET exported_at = GREATEST(now(), modified), client_ref=v_client_ref  WHERE id=v_topic_id ;	
