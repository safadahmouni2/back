...
BEGIN
		...
		DECLARE v_project,
				v_parent VARCHAR(100) CHARACTER SET UTF8;
	   
	   DECLARE  v_client_ref VARCHAR(100) CHARACTER SET UTF8;

		DECLARE v_creator,
		        v_modifier VARCHAR(250) CHARACTER SET UTF8;

		DECLARE v_ds_meeting_url VARCHAR(500) CHARACTER SET UTF8;
        ...
        DECLARE  cursor_data CURSOR FOR 
          ...
          SP.created,
          SP.modified,
          COALESCE(SP.client_ref, CONCAT(KUNDE_REF_PREFIX, SP.id)) AS client_ref
        FROM sprint AS SP
		 LEFT JOIN sprint AS PSP ON SP.parent = PSP.id
        ...
        read_loop: LOOP
			...
			IF (v_count =0)THEN
				...
				UPDATE sprint SET exported_at = GREATEST(now(), modified), client_ref=v_client_ref WHERE id=v_sprint_id ;		
			...
			ELSE
				...
				WHERE kunde_ref= v_client_ref limit 1;
				-- log text
				UPDATE sprint 
				SET 
