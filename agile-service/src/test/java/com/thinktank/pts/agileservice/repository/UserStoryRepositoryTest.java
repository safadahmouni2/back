...
class UserStoryRepositoryTest extends AbstractPersistenceTestBase {
	...
	private static final String FIELD_NAME_USER_STORIES = "userStories";

	private static final String FIELD_NAME_TICKET_ID = "ticketId";
	...
	void findAllUserStoriesBySprint_success() {
		// ARRANGE
		EasyRandomParameters parameters = buildEasyRandomParameters();
		parameters.excludeField(FieldPredicates.named(FIELD_NAME_TICKET_ID).and(FieldPredicates.inClass(Sprint.class)));
		parameters.excludeField(
				FieldPredicates.named(FIELD_NAME_TICKET_ID).and(FieldPredicates.inClass(UserStory.class)));
	...
	void findAllUserStoriesByTopic_success() {
		// ARRANGE
		EasyRandomParameters parameters = buildEasyRandomParameters();
		parameters.excludeField(FieldPredicates.named(FIELD_NAME_TICKET_ID).and(FieldPredicates.inClass(Topic.class)));
		parameters.excludeField(
				FieldPredicates.named(FIELD_NAME_TICKET_ID).and(FieldPredicates.inClass(UserStory.class)));
	...
	private EasyRandomParameters buildEasyRandomParameters() {
		...
		result.excludeField(FieldPredicates.named(FIELD_NAME_USER_STORIES).and(FieldPredicates.inClass(Sprint.class)));

		result.excludeField(FieldPredicates.named(FIELD_NAME_USER_STORIES).and(FieldPredicates.inClass(Feature.class)));

		result.excludeField(FieldPredicates.named(FIELD_NAME_USER_STORIES).and(FieldPredicates.inClass(Topic.class)));
		return result;
	}
